...
family_info = readxl::read_xlsx("data/from_xin/Related People In iPOP_from.sophia.xlsx", 
                                col_names = FALSE) 
setwd("data_analysis/combine_microbiome/distance/skin/")
...
#       subdat = prune_taxa(allGenus == temp_genus, physeq_skin)
...
#   dplyr::bind_rows()
# 
# save(skin_braydist_by_genus,
#      file = "skin_braydist_by_genus",
#      compress = "xz")
...
head(skin_braydist_by_genus)
table(skin_braydist_by_genus$subject_id1)

####if the diff days affect the within and between distance
masstools::setwd_project()
setwd("data_analysis/combine_microbiome/distance/skin/")

all_genus = skin_braydist_by_genus$genus %>% 
    unique()

# save(all_genus, file = "all_genus")

# final_result =
#   purrr::map(1:length(unique(skin_braydist_by_genus$genus)), function(idx) {
#     cat(idx, " ")
#     temp_genus = unique(skin_braydist_by_genus$genus)[idx]
#     temp_data_between =
#       skin_braydist_by_genus %>%
#       dplyr::filter(genus == unique(skin_braydist_by_genus$genus)[idx]) %>%
#       dplyr::filter(dist != 1) %>%
