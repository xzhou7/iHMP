...
skin_jaccard_by_genus = 
  ...
  .f = function(i) {
    ...
    subdat = tryCatch(prune_taxa(allGenus == temp_genus, physeq_skin), 
                      error = function(e) return(NA))
    subdat = tryCatch(prune_taxa(taxa_sums(subdat) > 0, subdat), 
                      error = function(e) return(NA))
    subdat = tryCatch(prune_samples(sample_sums(subdat) > 0, subdat),
                      error = function(e) return(NA))
    subdat = tryCatch(prune_samples(sample_names(subdat) %in% sample_info$KitID[match(i[2:3], sample_info$sample_id)], subdat),
                      error = function(e) return(NA))
...
save(skin_jaccard_by_genus,
     file = "skin_jaccard_by_genus",
     compress = "xz")
