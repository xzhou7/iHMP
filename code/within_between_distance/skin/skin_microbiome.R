...
dir.create("data_analysis/skin_microbiome/")
dir.create("data_analysis/skin_microbiome/cladogram/")
setwd("data_analysis/skin_microbiome/cladogram/")

load("bc_dist_by_genus_SK.rda")
bc_dist_skin = bc_dist_by_genus
rm(list = "bc_dist_by_genus")
dim(bc_dist_skin)
unique(bc_dist_skin$genus)
...
expression_data = physeqGenus_SK@otu_table@.Data %>%
  t() %>%
  as.data.frame()

variable_info = as.data.frame(physeqGenus_SK@tax_table@.Data)

sample_info = get_variable(physeq = physeqGenus_SK)

####remove some variables
###change to relative counts
##only remain taxs with median > 1e-5
# physeqGenus_SK2 = filter_taxa(physeq = physeqGenus_SK, flist = function(x) median(x) > 1e-5, prune =  TRUE)
##remove some variables which genus are not in temp_data
# physeqGenus_SK2 = filter_taxa(physeq = physeqGenus_SK, 
#                               flist = function(x) median(x) > 1e-5, prune =  TRUE)
#                               
# physeqGenus_SK2 =
#   subset_taxa(physeq = physeqGenus_SK, Genus %in% grep("Unclassified",temp_data$Genus, value = TRUE, invert = TRUE))
...
# ##
# physeqGenus_SK2 =
#   subset_taxa(physeq = physeqGenus_SK2, !Genus %in% "Unclassified_Actinobacteria")
...
ggsave(final_p, filename = "skin.tree.pdf", width = 12, height = 12)
