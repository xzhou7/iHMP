...
#     dplyr::bind_rows()
# 
# save(stool_braydist_by_genus,
#      file = "stool_braydist_by_genus",
#      compress = "xz")
...
# final_result =
#   purrr::map(1:length(unique(stool_braydist_by_genus$genus)), function(idx) {
#     cat(idx, " ")
#     temp_genus = unique(stool_braydist_by_genus$genus)[idx]
#     temp_data_between =
#       stool_braydist_by_genus %>%
#       dplyr::filter(genus == unique(stool_braydist_by_genus$genus)[idx]) %>%
#       dplyr::filter(dist != 1) %>%
