...
#Build costom function (HMT, Healthy Median Test) to extract median value of healthy baselines per subject per variable
  ...
  myHMT <- function(ds, m, fdr, trans){
    ...
    htb.cl = merge(htb, sc[, c(1,5,11,12)], by.x = "subject", by.y = "SubjectID") #Add classifications
  ...
  #Host
  hmt.ck = myHMT(ck.df[, c(4:65, 70, 75)], 62, 0.1, "none") #6 for 20% FDR, 1 at 10%
  hmt.clinic = myHMT(clinic.df[, c(2:53, 58)], 51, 0.1, "none") #5 for 10% FDR
  hmt.metb = myHMT(metbcr.df[, c(2:726, 731)], 724, 0.1, "log2")  #19 for 10% FDR
  hmt.rna = myHMT(rnaseq.log.df[, c(2:10348, 10353)], 10346, 0.2, "none") # 0 for 20% FDR
  hmt.prot = myHMT(swathprot.PCR.df[, c(2:303, 304, 309)], 302, 0.1, "none") #4 for 10% FDR
  #Microbiome
  hmt.st = myHMT(st.df[, c(2:97, 103, 102)], 96, 0.1, "arcsin") #for 1% FDR, 14 taxonomic levels
  hmt.ns = myHMT(ns.df[, c(2:81, 87, 86)], 80, 0.2, "arcsin") #0 for 20% FDR
  hmt.stKO = myHMT(stKO.df[, c(2:362, 368, 367)], 361, 0.1, "arcsin/1000") #for 5% FDR, 32 KOs, 10% FDR, 50 KOs
  ...
  hmt.tb = merge(merge(merge(merge(merge(hmt.clinic, hmt.ck[, -3], by = "SubjectID", all = TRUE), hmt.metb[, -21], by = "SubjectID", all = TRUE), hmt.st[, -27], by = "SubjectID", all = TRUE), hmt.prot[, -6], by = "SubjectID", all = TRUE), hmt.stKO[, -52], by = "SubjectID", all = TRUE)
  hmt.tb$SSPG[c(6,42)] <- c(224,71) #For subject 69-030 and 69-106
  ...
  pd = t(apply(hmt.tb[1:65, c(2:6, 8:27, 36:45)], 2, function(x) scale(as.numeric(x))))
  rownames(pd) = colnames(hmt.tb)[c(2:6, 8:27, 36:45)]
  ...
  var_annotation = c(rep(c("pink"), times=5), rep(c("red"), times=1), rep(c("grey"), times=19), rep(c("darkgreen"), times=6), rep(c("orange"), times=4))
