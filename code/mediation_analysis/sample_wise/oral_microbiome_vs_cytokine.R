...
{
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/oral_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/oral_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/oral_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/cytokine_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/cytokine_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/cytokine_sample_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set/oral_microbiome_vs_cytokine/oral_microbiome_cytokine_lm_adjusted_cor_spearman"
)
...
masstools::setwd_project()
dir.create(
  "data_analysis/mediation_analysis/sample_wise/oral_microbiome_vs_cytokine",
  recursive = TRUE
)
setwd("data_analysis/mediation_analysis/sample_wise/oral_microbiome_vs_cytokine")
...
for (i in 1:nrow(oral_microbiome_cytokine_cor)) {
  ...
  temp_data <-
    data.frame(
      microbiome = as.numeric(oral_microbiome_expression_data[microbiome_id,]),
  ...
  temp_data <-
    temp_data %>%
    dplyr::mutate(Gender = case_when(Gender == "F" ~ 0,
                                     Gender == "M" ~ 1))
  
  glm_reg1 =
    glm(internal_ome ~ microbiome + Gender + Adj.age + BMI,
        family = gaussian,
        temp_data)
  ...
  for (j in 1:nrow(temp_oral_microbiome_phenotype_cor)) {
    ...
    glm_reg2 =
      glm(phenotype ~ microbiome + internal_ome + Gender + Adj.age + BMI,
          family = gaussian,
          temp_data2)
...
mediation_result <-
  mediation_result %>%
  dplyr::left_join(
    oral_microbiome_cytokine_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
    by = c("treat" = "microbiome",
           "mediator" = "metabolite")
  ) %>%
  dplyr::rename(
    treat_mediator_cor = cor,
    treat_mediator_p = p,
    treat_mediator_p_adjust = p_adjust
  ) %>%
  dplyr::left_join(
    oral_microbiome_phenotype_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
    by = c("treat" = "microbiome",
           "phenotype" = "metabolite")
  ) %>%
  dplyr::rename(
    treat_phenotype_cor = cor,
    treat_phenotype_p = p,
    treat_phenotype_p_adjust = p_adjust
  ) %>%
  dplyr::left_join(
    cytokine_phenotype_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
    by = c("mediator" = "microbiome",
           "phenotype" = "metabolite")
  ) %>%
  dplyr::rename(
    mediator_phenotype_cor = cor,
...
for (i in 1:nrow(mediation_result)) {
  ...
  glm_reg1 =
    glm(phenotype ~ microbiome + Gender + Adj.age + BMI,
        family = gaussian,
        temp_data)
...
ggsave(plot,
       filename = "mediation_analysis.pdf",
       width = 7,
       height = 7)
