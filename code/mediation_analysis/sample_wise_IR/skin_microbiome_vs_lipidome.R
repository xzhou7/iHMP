...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/skin_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/skin_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/skin_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/lipidome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/lipidome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/lipidome_sample_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_lipidome/skin_microbiome_lipidome_lm_adjusted_cor_spearman"
)

skin_microbiome_lipidome_cor <- 
...
dim(skin_microbiome_lipidome_cor)
dim(skin_microbiome_phenotype_cor)

unique(skin_microbiome_lipidome_cor$microbiome)
unique(skin_microbiome_phenotype_cor$microbiome)

intersect_id <- 
  intersect(unique(skin_microbiome_lipidome_cor$microbiome),
            unique(skin_microbiome_phenotype_cor$microbiome))
...
######work directory
masstools::setwd_project()
dir.create("data_analysis/mediation_analysis/sample_wise_IR/skin_microbiome_vs_lipidome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/skin_microbiome_vs_lipidome")

###here we use the lm_adjusted_cor
sum(skin_microbiome_lipidome_cor$p_adjust < 0.2)
...
for(i in 1:nrow(skin_microbiome_lipidome_cor)) {
  cat(i, "\n")
  microbiome_id = skin_microbiome_lipidome_cor$microbiome[i]
  internal_ome_id = skin_microbiome_lipidome_cor$metabolite[i]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(skin_microbiome_lipidome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  shape = 21,
                  show.legend = TRUE) +
  scale_fill_manual(values = c(omics_color[c("Skin microbiome", "Cytokine")], phenotype_color)) +
  scale_color_manual(values = c(omics_color[c("Skin microbiome", "Cytokine")], phenotype_color)) +
  geom_node_text(
    aes(
      x = x * 1.05,
      y = y * 1,
