...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/stool_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/stool_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/stool_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/metabolome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/metabolome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/metabolome_sample_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/stool_microbiome_vs_metabolome/stool_microbiome_metabolome_lm_adjusted_cor_spearman"
)

stool_microbiome_metabolome_cor <- 
...
dim(stool_microbiome_metabolome_cor)
dim(stool_microbiome_phenotype_cor)

unique(stool_microbiome_metabolome_cor$microbiome)
unique(stool_microbiome_phenotype_cor$microbiome)

intersect_id <- 
  intersect(unique(stool_microbiome_metabolome_cor$microbiome),
            unique(stool_microbiome_phenotype_cor$microbiome))
...
######work directory
masstools::setwd_project()
dir.create("data_analysis/mediation_analysis/sample_wise_IR/stool_microbiome_vs_metabolome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/stool_microbiome_vs_metabolome")

###here we use the lm_adjusted_cor
sum(stool_microbiome_metabolome_cor$p_adjust < 0.2)
...
for(i in 1:nrow(stool_microbiome_metabolome_cor)) {
  cat(i, "\n")
  microbiome_id = stool_microbiome_metabolome_cor$microbiome[i]
  internal_ome_id = stool_microbiome_metabolome_cor$metabolite[i]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(stool_microbiome_metabolome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
my_layout1$y[my_layout1$class == "Stool microbiome"] <-
  seq(from = 1, to = 100, length.out = sum(my_layout1$class == "Stool microbiome"))
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  show.legend = TRUE) +
  scale_fill_manual(
    values = c(omics_color[c("Stool microbiome", "Metabolite")], phenotype_color)
  ) +
  scale_color_manual(
    values = c(omics_color[c("Stool microbiome", "Metabolite")], phenotype_color)
  ) +
