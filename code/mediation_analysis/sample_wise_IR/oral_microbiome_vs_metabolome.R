...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/oral_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/oral_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/oral_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/metabolome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/metabolome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/metabolome_sample_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/oral_microbiome_vs_metabolome/oral_microbiome_metabolome_lm_adjusted_cor_spearman"
)
...
intersect_id <- 
  intersect(unique(oral_microbiome_metabolome_cor$microbiome),
            unique(oral_microbiome_phenotype_cor$microbiome))
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/mediation_analysis/sample_wise_IR/oral_microbiome_vs_metabolome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/oral_microbiome_vs_metabolome")

###here we use the lm_adjusted_cor
sum(oral_microbiome_metabolome_cor$p_adjust < 0.2)
...
for(i in 1:nrow(oral_microbiome_metabolome_cor)) {
  cat(i, "\n")
  microbiome_id = oral_microbiome_metabolome_cor$microbiome[i]
  internal_ome_id = oral_microbiome_metabolome_cor$metabolite[i]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(oral_microbiome_metabolome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
my_layout1$y[my_layout1$class == "Oral microbiome"] <-
  seq(from = 1, to = 100, length.out = sum(my_layout1$class == "Oral microbiome"))
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  show.legend = TRUE) +
  scale_fill_manual(
    values = c(omics_color[c("Oral microbiome", "metabolome")], phenotype_color)
  ) +
  scale_color_manual(
    values = c(omics_color[c("Oral microbiome", "metabolome")], phenotype_color)
  ) +
