...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/nasal_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/nasal_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/nasal_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/proteome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/proteome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/proteome_sample_info"
  )
}
...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_phenotype_sample_wise/phenotype_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_phenotype_sample_wise/phenotype_sample_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_phenotype_sample_wise/phenotype_variable_info"
  )
}
...
###finally, for stool microbiome, 107 genus, for proteome, 302 proteins
###563 samples
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_proteome/nasal_microbiome_proteome_lm_adjusted_cor_spearman"
)

nasal_microbiome_proteome_cor <- 
  nasal_microbiome_proteome_lm_adjusted_cor_spearman %>% 
  dplyr::filter(p_adjust < 0.2)

load(
  "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_phenotype_sample_wise/nasal_microbiome_phenotype_lm_adjusted_cor_spearman"
)

nasal_microbiome_phenotype_cor <- 
  nasal_microbiome_phenotype_lm_adjusted_cor_spearman %>% 
  dplyr::filter(p_adjust < 0.2)


load(
  "data_analysis/correlation_network/whole_data_set_IR/proteome_vs_phenotype_sample_wise/proteome_phenotype_lm_adjusted_cor_spearman"
)

proteome_phenotype_cor <- 
  proteome_phenotype_lm_adjusted_cor_spearman %>% 
  dplyr::filter(p_adjust < 0.2)

dim(nasal_microbiome_proteome_cor)
dim(nasal_microbiome_phenotype_cor)

unique(nasal_microbiome_proteome_cor$microbiome)
unique(nasal_microbiome_phenotype_cor$microbiome)

intersect_id <- 
intersect(unique(nasal_microbiome_proteome_cor$microbiome),
          unique(nasal_microbiome_phenotype_cor$microbiome))
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/mediation_analysis/sample_wise_IR/nasal_microbiome_vs_proteome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/nasal_microbiome_vs_proteome")

###here we use the lm_adjusted_cor
sum(nasal_microbiome_proteome_cor$p_adjust < 0.2)

library(mediation)

plot(density(as.numeric(proteome_expression_data[1,])))
...
for(i in 1:nrow(nasal_microbiome_proteome_cor)) {
  cat(i, "\n")
  microbiome_id = nasal_microbiome_proteome_cor$microbiome[i]
  internal_ome_id = nasal_microbiome_proteome_cor$metabolite[i]

  temp_data <-
  data.frame(microbiome = as.numeric(nasal_microbiome_expression_data[microbiome_id,]),
             internal_ome = as.numeric(proteome_expression_data[internal_ome_id,]),
             nasal_microbiome_sample_info
             )
  ...
  temp_nasal_microbiome_phenotype_cor =
    nasal_microbiome_phenotype_cor %>%
    dplyr::filter(microbiome %in% microbiome_id)

  if(nrow(temp_nasal_microbiome_phenotype_cor) == 0){
    next()
  }

  for (j in 1:nrow(temp_nasal_microbiome_phenotype_cor)) {
    cat(j, " ")
    phenotype_id <-
      temp_nasal_microbiome_phenotype_cor$metabolite[j]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(nasal_microbiome_proteome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
                   by = c("treat" = "microbiome",
                          "mediator" = "metabolite")) %>%
  dplyr::rename(treat_mediator_cor = cor,
                treat_mediator_p = p,
                treat_mediator_p_adjust = p_adjust) %>%
  dplyr::left_join(nasal_microbiome_phenotype_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
                   by = c("treat" = "microbiome",
                          "phenotype" = "metabolite")) %>%
  dplyr::rename(treat_phenotype_cor = cor,
                treat_phenotype_p = p,
                treat_phenotype_p_adjust = p_adjust) %>%
  dplyr::left_join(proteome_phenotype_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
node_data =
  ...
  dplyr::mutate(
    class1 = case_when(
      node %in% nasal_microbiome_variable_info$variable_id ~ "Nasal microbiome",
...
my_layout$y[my_layout$class == "proteome"] <- 5
...
my_layout1$y[my_layout1$class == "Nasal microbiome"] <-
  seq(
    from = 1,
    to = 100,
    length.out = sum(my_layout1$class == "Nasal microbiome")
  )

my_layout1$y[my_layout1$class == "proteome"] <-
  seq(
    from = 1,
    to = 100,
    length.out = sum(my_layout1$class == "proteome")
  )
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  shape = 21,
                  show.legend = TRUE) +
  scale_fill_manual(values = c(omics_color[c("Nasal microbiome", "Proteome")], phenotype_color)) +
  scale_color_manual(values = c(omics_color[c("Nasal microbiome", "Proteome")], phenotype_color)) +
  geom_node_text(
    aes(
      x = x * 1.05,
      y = y * 1,
      label = true_name,
      hjust = ifelse(class1 == "proteome", 1, 0),
      size = 3,
      colour = class1
    ),
    size = 3,
