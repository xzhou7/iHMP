...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/skin_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/skin_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/skin_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/proteome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/proteome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/proteome_sample_info"
  )
}
...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_phenotype_sample_wise/phenotype_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_phenotype_sample_wise/phenotype_sample_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_phenotype_sample_wise/phenotype_variable_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_proteome/skin_microbiome_proteome_lm_adjusted_cor_spearman"
)

skin_microbiome_proteome_cor <- 
  skin_microbiome_proteome_lm_adjusted_cor_spearman %>% 
  dplyr::filter(p_adjust < 0.2)

load(
  "data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_phenotype_sample_wise/skin_microbiome_phenotype_lm_adjusted_cor_spearman"
)
...
dim(skin_microbiome_proteome_cor)
dim(skin_microbiome_phenotype_cor)

unique(skin_microbiome_proteome_cor$microbiome)
unique(skin_microbiome_phenotype_cor$microbiome)

intersect_id <- 
  intersect(unique(skin_microbiome_proteome_cor$microbiome),
            unique(skin_microbiome_phenotype_cor$microbiome))
...
######work directory
masstools::setwd_project()
dir.create("data_analysis/mediation_analysis/sample_wise_IR/skin_microbiome_vs_proteome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/skin_microbiome_vs_proteome")

###here we use the lm_adjusted_cor
sum(skin_microbiome_proteome_cor$p_adjust < 0.2)
...
for(i in 1:nrow(skin_microbiome_proteome_cor)) {
  cat(i, "\n")
  microbiome_id = skin_microbiome_proteome_cor$microbiome[i]
  internal_ome_id = skin_microbiome_proteome_cor$metabolite[i]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(skin_microbiome_proteome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
my_layout1$y[my_layout1$class == "Skin microbiome"] <-
  seq(
    from = 1,
    to = 100,
    length.out = sum(my_layout1$class == "Skin microbiome")
  )
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  shape = 21,
                  show.legend = TRUE) +
  scale_fill_manual(values = c(omics_color[c("Skin microbiome", "Proteome")], phenotype_color)) +
  scale_color_manual(values = c(omics_color[c("Skin microbiome", "Proteome")], phenotype_color)) +
  geom_node_text(
    aes(
      x = x * 1.05,
      y = y * 1,
