...
{
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/nasal_microbiome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/nasal_microbiome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/nasal_microbiome_sample_info"
  )
  
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/metabolome_expression_data"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/metabolome_variable_info"
  )
  load(
    "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/metabolome_sample_info"
  )
}
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/nasal_microbiome_vs_metabolome/nasal_microbiome_metabolome_lm_adjusted_cor_spearman"
)
...
load(
  "data_analysis/correlation_network/whole_data_set_IR/metabolome_vs_phenotype_sample_wise/metabolome_phenotype_lm_adjusted_cor_spearman"
)
...
intersect_id <- 
intersect(unique(nasal_microbiome_metabolome_cor$microbiome),
          unique(nasal_microbiome_phenotype_cor$microbiome))
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/mediation_analysis/sample_wise_IR/nasal_microbiome_vs_metabolome",
           recursive = TRUE)
setwd("data_analysis/mediation_analysis/sample_wise_IR/nasal_microbiome_vs_metabolome")

###here we use the lm_adjusted_cor
sum(nasal_microbiome_metabolome_cor$p_adjust < 0.2)
...
for(i in 1:nrow(nasal_microbiome_metabolome_cor)) {
  cat(i, "\n")
  microbiome_id = nasal_microbiome_metabolome_cor$microbiome[i]
  internal_ome_id = nasal_microbiome_metabolome_cor$metabolite[i]
...
mediation_result <-
mediation_result %>%
  dplyr::left_join(nasal_microbiome_metabolome_lm_adjusted_cor_spearman %>% dplyr::select(-p_adjust2),
...
for(i in 1:nrow(mediation_result)){
  ...
  microbiome_id = mediation_result$treat[i]
  internal_ome_id = mediation_result$mediator[i]
  phenotype_id = mediation_result$phenotype[i]
...
node_data =
  ...
  dplyr::mutate(
    class1 = case_when(
      ...
      node %in% metabolome_variable_info$variable_id ~ "metabolome",
...
my_layout$y[my_layout$class == "metabolome"] <- 5
...
my_layout1$y[my_layout1$class == "Nasal microbiome"] <-
  seq(from = 1, to = 100, length.out = sum(my_layout1$class == "Nasal microbiome"))

my_layout1$y[my_layout1$class == "metabolome"] <-
  seq(from = 1, to = 100, length.out = sum(my_layout1$class == "metabolome"))
...
plot <-
  ...
  geom_node_point(aes(fill = class1,
                  ...
                  show.legend = TRUE) +
  scale_fill_manual(
    values = c(omics_color[c("Nasal microbiome", "metabolome")], phenotype_color)
  ) +
  scale_color_manual(
    values = c(omics_color[c("Nasal microbiome", "metabolome")], phenotype_color)
  ) +
  geom_node_text(
    aes(
      ...
      y = y * 1,
      label = true_name,
      hjust = ifelse(class1 == "metabolome", 1, 0),
      size = 3,
      colour = class1
    ),
    size = 3,
